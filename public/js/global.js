...
function updateTweet() {
  $.ajax({
    ...
    success: function(data) {
      // console.log(data);
      $("#avatar").attr("src", data.profile_image_uri);
      $("#author-screen-name-link").attr("href", "https://twitter.com/" + data.author_screen_name);
      $("#author-screen-name").text(data.author_screen_name);
      $("#author-name").text(data.author_name);
      $("#text").text(data.text);
      $("#date").text(data.date);
      $("#favorite-count").text(data.favorite_count);
      $("#retweet-count").text(data.retweet_count);
      $("#tweet-id").text(data.tweet_id);
    }
  });

  var tweet = $("#text");
  var sampleString = ""
  var tweetContent = tweet.text().match(/@\w+/g);
  // var resultSet = 
  console.log("Referenced users: ");
}

function deleteTweet() {
  if(confirm('Delete tweet with tweet id ' + $("#tweet-id").text() + '?')) {
    $.ajax({
      url: '/deleteTweet',
      data: { tweet_id: $("#tweet-id").text() },
      dataType: 'text',
      type: 'post',
      aysnc: false,
      success: function(data) {
